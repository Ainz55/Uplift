...
def train_tarnet(
    ...
    *,
    ...
    ds_train = TensorDataset(
        ...
        torch.tensor(y_log_train),
        torch.tensor(t_train.astype(np.int8)),
    )
    loader = DataLoader(ds_train, batch_size=batch_size, shuffle=True, num_workers=0)
    ...
    for epoch in range(1, epochs + 1):
        ...
        with torch.no_grad():
            ...
            val_loss = tarnet_loss(y_t_v, y_c_v, prop_v, y_val_t, t_val_t, nonzero_weight=nonzero_weight).item()

        if epoch % 10 == 0:
            logger.info(
                "Epoch %3d | train=%.4f | val=%.4f | lr=%.2e",
                epoch, np.mean(train_losses), val_loss, scheduler.get_last_lr()[0],
            )
    ...
    if best_state is not None:
        ...
        logger.info("Loaded best weights (val_loss=%.4f)", best_val_loss)

    # Attach scaler to model for inference
...
def run_train(cfg: argparse.Namespace) -> None:
    ...
    logger.info("Val uplift@10: %.4f", score)

    # Save model
    model_path = output_dir / "tarnet.pt"
    torch.save(
        {"state_dict": model.state_dict(), "scaler": scaler, "feature_cols": feature_cols},
        model_path,
    )
    ...
    test_uplift = model.predict_uplift(X_test_s)
...
def main() -> None:
    ...
    parser.add_argument("--no-dragonnet", action="store_true")
    # Ensemble params
